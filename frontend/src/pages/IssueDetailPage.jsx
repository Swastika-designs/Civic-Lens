...
const STATUS_ICONS = { 'Reported': '📥', 'In Review': '🔍', 'In Progress': '🔧', 'Resolved': '✅' };
...
export default function IssueDetailPage() {
  ...
  if (!issue) return <div className="flex justify-center items-center min-h-screen text-gray-400">Issue not found</div>;
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="flex items-center justify-between px-4 pt-8 pb-3">
        ...
        <div className="flex gap-3">
          <button onClick={() => navigator.share?.({ title: issue.title, url: window.location.href })} className="text-gray-500"><Share2 size={18} /></button>
      ...
      {issue.photo?.url && (
        <div className="relative mx-4 rounded-2xl overflow-hidden mb-4">
          <img
            ...
            alt={issue.title} className="w-full h-52 object-cover" />
          <span className={`absolute top-3 left-3 ${STATUS_CLASSES[issue.status]}`}>{issue.status}</span>
        </div>
      )}
      ...
      <div className="px-4">
        ...
        <div className="flex items-center gap-1 text-gray-400 text-sm mt-1">
          <MapPin size={12} /> {issue.location?.address} • Reported {new Date(issue.createdAt).toLocaleDateString()}
        </div>

        {/* Upvote */}
        <button onClick={handleUpvote}
          className={`mt-3 flex items-center gap-2 text-sm font-medium px-4 py-2 rounded-full border transition-colors ${upvoted ? 'bg-primary text-white border-primary' : 'border-gray-200 text-gray-600 hover:border-primary'}`}>
            ...
            {issue.timeline?.map((event, i) => (
              <div key={i} className="flex gap-3 mb-4 last:mb-0">
                <div className="flex flex-col items-center">
                  <div className={`w-7 h-7 rounded-full flex items-center justify-center text-sm flex-shrink-0 ${i === issue.timeline.length - 1 ? 'bg-primary text-white' : 'bg-gray-200 text-gray-500'}`}>
                    {STATUS_ICONS[event.status] || '•'}
                  </div>
                  {i < issue.timeline.length - 1 && <div className="w-0.5 h-full bg-gray-200 mt-1" />}
                </div>
                <div>
                  <p className={`font-medium text-sm ${i === issue.timeline.length - 1 ? 'text-primary' : ''}`}>{event.title}</p>
                  <p className="text-gray-500 text-xs mt-0.5">{event.description}</p>
                  <p className="text-gray-400 text-xs mt-0.5">{new Date(event.timestamp).toLocaleString()}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        {issue.officialUpdate?.message && (
          <div className="bg-blue-50 border border-blue-100 rounded-2xl p-4 mt-4">
            ...
            <div className="bg-white rounded-xl p-3">
              ...
              <p className="text-gray-400 text-xs mt-1">{new Date(issue.officialUpdate.timestamp).toLocaleString()}</p>
            </div>
          </div>
        )}
        ...
        <div className="bg-blue-50 rounded-2xl p-4 mt-4 mb-6">
          ...
          <p className="text-gray-500 text-xs mb-3">Enable notifications to get real-time alerts when the status changes.</p>
          <button className="btn-primary text-sm py-2.5 w-full">
